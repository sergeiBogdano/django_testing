...
class TestLogic(NoteTestBase):
    def setUp(self):
        super().setUp()
        self.edit_url = EDIT_NOTE_URL.format(slug=self.note.slug)
        self.delete_url = DELETE_NOTE_URL.format(slug=self.note.slug)
    ...
    def test_anonymous_user_cannot_create_note(self):
        ...
        self.assertEqual(response.status_code, HTTPStatus.FOUND)
        self.assertEqual(response.url, self.expected_redirect_add_note_url)
    ...
    def test_user_can_edit_own_note(self):
        response = self.logged_in_client.post(self.edit_url, self.note_data)
        self.assertEqual(response.status_code, HTTPStatus.FOUND)
    ...
    def test_user_can_delete_own_note(self):
        ...
        response = self.logged_in_client.post(self.delete_url)
        self.assertEqual(response.status_code, HTTPStatus.FOUND)
        self.assertFalse(Note.objects.filter(id=self.note.id).exists())
        self.assertEqual(Note.objects.count(), initial_note_count - 1)

    def test_user_cannot_delete_others_note(self):
        response = self.logged_in_client_non_author.post(self.delete_url)
